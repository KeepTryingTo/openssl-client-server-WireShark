...
void initSSL()
{
    ...
    r = SSL_CTX_use_certificate_file(g_sslCtx, cert_file, SSL_FILETYPE_PEM);
    check0(r <= 0, "SSL_CTX_use_certificate_file %s failed", cert_file);
    r = SSL_CTX_use_PrivateKey_file(g_sslCtx, private_key, SSL_FILETYPE_PEM);
    check0(r <= 0, "SSL_CTX_use_PrivateKey_file %s failed", private_key);
